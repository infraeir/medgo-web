...
class InputChipMedgo extends StatefulWidget {
  ...
  @override
  State<InputChipMedgo> createState() => _InputChipMedgoState();
}

class _InputChipMedgoState extends State<InputChipMedgo> {
  ...
  Widget build(BuildContext context) {
    ...
    // Inactive + Pressed + Selected
    else if (widget.isInactive && isPressed && widget.selectedChip) {
    ...
    // Inactive + Selected
    else if (widget.isInactive && widget.selectedChip) {
      ...
      boxShadow = [
        const BoxShadow(
          blurRadius: 5,
          offset: Offset(0, 3),
          color: Color(0x4D000000), // black opacity 0.3
        ),
      ];
    }
    // Inactive + Pressed
    else if (widget.isInactive && isPressed) {
    ...
    // Inactive + Hover
    else if (widget.isInactive && isHovered) {
    ...
    else if (widget.isInactive) {
      backgroundColor = colorAlternate; // #E0E3E7
      borderColor = colorPrimary; // #004155
      textColor = colorSecondaryText; // #57636C
      boxShadow = [
        const BoxShadow(
          blurRadius: 5,
          offset: Offset(0, 3),
    ...
    if (isFocused && !widget.isDisabled) {
      if (widget.isInactive) {
        // Para chips inativos, apenas adiciona a sombra de foco
        boxShadow = [
          const BoxShadow(
            blurRadius: 5,
            offset: Offset(0, 3),
        ...
        // Para chips normais, aplica o estilo completo de foco
        boxShadow = [
          const BoxShadow(
            blurRadius: 5,
            offset: Offset(0, 3),
    ...
    return Focus(
      ...
      onKey: (node, event) {
        if (!widget.isDisabled &&
          ...
          if (event is KeyDownEvent) {
            if (widget.isInactive && widget.onInactivePressed != null) {
      ...
      child: MouseRegion(
        ...
        child: GestureDetector(
          ...
          // Removemos o onTap que solicitava foco
